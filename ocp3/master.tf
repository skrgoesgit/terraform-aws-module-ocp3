###################################################
# EC2 - OpenShift Master

resource "aws_instance" "ocp3_master" {
  count                  = length(var.ocp3_master_instances)
  ...
  vpc_security_group_ids = [aws_security_group.sg_openshift_master.id]
  ...
  tags = merge(
    var.tags,
    {
      "Name" = var.ocp3_master_instances[count.index]
    },
  )
