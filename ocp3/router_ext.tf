###################################################
# EC2 - OpenShift Router (external access)

resource "aws_instance" "ocp3_router_ext" {
  count                  = length(var.ocp3_router_ext_instances)
  ...
  vpc_security_group_ids = [aws_security_group.sg_openshift_router_ext.id]
  ...
  tags = merge(
    var.tags,
    {
      "Name" = var.ocp3_router_ext_instances[count.index]
    },
  )
